<h3 align="center"> Olá, eu sou o Ronald</h3>

<h4 align="center">
Desenvolvedor Backend na 
<a href="https://onium.com.br/" target="_blank">ONIUM Tecnologia e Sistemas</a>
</h4>

<p align="center">
Atuo no core técnico da <strong><a href="https://onium.com.br/" target="_blank">ONIUM</a></strong>, desenvolvendo soluções de alta criticidade para 
<strong>multinacionais e grandes terminais logísticos</strong> (Containers e Commodities). 
Meu foco está em projetar arquiteturas <strong>escaláveis e seguras</strong> que sustentam operações de 
fluxo contínuo, onde <strong>alta disponibilidade</strong> não é um diferencial, mas um requisito básico.
</p>

---
[<img align="left" height="130px" width="130px" alt="Onium" src="https://github.com/user-attachments/assets/fe6bf38b-7249-46d9-8161-d5e2ce86fe36"/>](https://onium.com.br/)

**Desenvolvedor FullStack** \
[**Onium**](https://onium.com.br/) • 
Linguagens & Tecnologias: `C#`, `.NET 6/8`, `Angular`, `Git`, `SqlServer`, `OCI`

<br/>

<p align="center">
Especialista em ecossistema <strong>.NET</strong>, aplico diariamente conceitos de <strong>Clean Architecture</strong> 
e <strong>SOLID</strong> para garantir que o código não apenas funcione, mas seja resiliente. 
Sou responsável pela manutenção de integrações complexas e pela automação de processos de 
<strong>controle de acesso e gestão de dados</strong>, operando sob rigorosos fluxos de <strong>CI/CD</strong>.
</p>

<p align="center">
<em>Transformando regras de negócio complexas em sistemas eficientes, performáticos e de baixa latência.</em>
</p>

<h4 align="center">🛠️ Arquitetura de Tecnologias</h4>

<table align="center">
  <tr>
    <td align="center" width="220">
      <strong>Front-end</strong><br/><br/>
      <img src="https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" /><br/>
      <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/Razor-512BD4?style=for-the-badge&logo=dotnet&logoColor=white" />
    </td>
    <td align="center" width="250">
      <strong>Database & Cloud</strong><br/><br/>
      <img src="https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black" />
    </td>
    <td align="center" width="220">
      <strong>Back-end & DevTools</strong><br/><br/>
      <img src="https://img.shields.io/badge/.NET_Core-512BD4?style=for-the-badge&logo=dotnet&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=csharp&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
    </td>
  </tr>
</table>

<br/>

---

<h4>🎓 Formação & Aprendizado</h4>

<p>
Sou estudante de <strong>Sistemas de Informação</strong> pela faculdade
<strong>Isulpar</strong>, em Paranaguá – PR.
Especializado em <strong>Backend</strong>, estudo continuamente
<strong>bancos de dados autônomos</strong>, <strong>segurança em nuvem</strong> e arquitetura de sistemas.
Estou sempre em busca de novos desafios para elevar a maturidade técnica das soluções que desenvolvo.
</p>

<h4 align="center">📫 Vamos nos conectar</h4>

<div align="center">
  <a href="https://www.linkedin.com/in/dev-ronald/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Ronald%20Carvalho-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" height="40"/>
  </a>
</div>
